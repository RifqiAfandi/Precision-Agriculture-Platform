...
export function GreenhouseCard({ greenhouse, isSelected, onClick }) {
  const statusConfig = {
    optimal: { color: "bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400", label: "Berkembang Baik" },
    warning: { color: "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400", label: "Perlu Perhatian" },
    danger: { color: "bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400", label: "Perlu Tindakan" },
  };
      ...
      <CardContent className="p-2 sm:p-3 md:p-4">
        <div className="flex items-start justify-between flex-wrap gap-2">
          <div className="flex items-center space-x-2 sm:space-x-3">
            <div className="w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full bg-green-100 dark:bg-slate-700 flex items-center justify-center flex-shrink-0">
              <Home className="w-3.5 h-3.5 sm:w-4 sm:h-4 md:w-5 md:h-5 text-green-600" />
            </div>
            <div>
              <h3 className="font-semibold text-xs sm:text-sm md:text-base text-gray-900 dark:text-gray-100">{greenhouse.name}</h3>
              <p className="text-[10px] sm:text-xs md:text-sm text-gray-500 dark:text-gray-400">{greenhouse.location}</p>
            </div>
          </div>
        ...
        <div className="mt-2 sm:mt-3 md:mt-4 grid grid-cols-2 gap-2 sm:gap-2 md:gap-3">
          ...
          <div className="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm">
            ...
            <div className="min-w-0">
              <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">Kelembaban</p>
              <p className="font-semibold text-xs sm:text-sm text-gray-900 dark:text-gray-100 truncate">
